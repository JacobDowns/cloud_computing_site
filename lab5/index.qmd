...
* **Windows (PowerShell):**
  ```powershell
  ssh-keygen -t ed25519 -C "admin-key" -f "$HOME\.ssh\jetstream2"
  ```
...
* **Windows (PowerShell):**
  ```powershell
  ssh-keygen -t ed25519 -C "github-deploy" -f "$HOME\.ssh\github_deploy_key"
  ```
...
1. Log into [Jetstream2 Exosphere](https://jetstream2.exosphere.app/).

> Note: You'll need to login with your NSF ACCESS credentials. The first time you login you'll need to click **Add allocation** and select our class allocation. 
