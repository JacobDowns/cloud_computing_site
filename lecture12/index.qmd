...
-   **The Scale:** Gigabytes to Exabytes. It scales "infinitely". You can always add more to your buckets.
-   **Rule of Thumb:** If your code uses standard file open/read/write operations (handling `.csv`, `.png`, or `.mp4` files), it likely belongs in Cloud Storage.
...
-   **However, cold storage charges you a "Retrieval Fee" every time you access a file.** 
- If you put a highly trafficked website logo in Archive storage, the retrieval fees will add up quickly.
...
::: {.callout-note collapse="false"}
## example.exe 
Suppose you are building the backend for a new multiplayer game.
...
- In cloud architecture, we often say that **"Storage is cheap, but movement is expensive."** 
- The network and access fees are often the "hidden" part of a cloud bill that can easily exceed the actual cost of storing the data itself.
...
- Imagine you host 10MB video in a Standard Cloud Storage bucket. Storing that file costs you less than a penny a month. 
- However, if your app goes viral and 1 million people download that video, you are suddenly paying for **10,000 GB of Internet Egress**. 
- This could easily cost around a grand in egress costs. 
- What looked like a cheap storage solution just generated a massive network bill!
:::

::: {.callout-note collapse="true"}
## aside.sys — What is a Service Account?
...
-   By default, buckets are entirely private. To allow apps permission to use the data, there are a couple of options. 
...
-   **The "Signed URL" Way** You keep the bucket private. No one can see the files except your App Engine code (using its Service Account badge). Your code then generates a temporary signed URL for the user's browser.
...
When your data is highly structured, strictly typed, and requires complex relationships (e.g., Users, Roles, Permissions, Transactions), you will likely reach for a Relational Database.

### Option A: Cloud SQL (The Standard)

- Cloud SQL is Google's managed service for MySQL, PostgreSQL, and SQL Server. 
- Google handles the backups, the software patching, and the hard drive provisioning automatically.
...
-   **Rule of Thumb:** Use this when you have strict data integrity needs and your schema is well-defined. 
...
> Note: I highly recommend running this in the Google Cloud Shell built into the web browser, as it comes pre-configured with the correct network authorizations to connect to Cloud SQL.Alternatively, feel free to use the UI to sping up a DB instance. 
...
-   **Rule of Thumb:** You will likely never use Spanner for a class project. It is designed for massive global enterprises (think global banking, or a large multiplayer game). 
...
- Instead of tables and rows, Firestore organizes data hierarchically into **Collections** and **Documents**:
-   **Document:** A lightweight record that contains fields, which map to values (very similar to a JSON object). 
-   **Collection:** A container for your documents. 
-   **The Hierarchy:** You can think of a Collection as a folder, and a Document as a text file inside that folder.
...
-   **Rule of Thumb:** This is a good choice for student projects. If you are building a flexible web or mobile app, Firestore is a good option.

::: {.callout-note collapse="false"}
## example.exe -- A Use Case
...
### Adding Data Manually (The Web Console)
- While you will typically write code to insert data, the easiest way to understand the NoSQL structure is to manually construct a document using the Google Cloud Web Console. 
- To ensure our data works perfectly with the App Engine code we will write in Part 6, we need to create a nested object (a dictionary within a dictionary) called a **Map**.

1. Navigate to **Firestore -> Databases** in the Google Cloud Console.
2. Click on your new `player-data` database to open the data viewer.
3. Click **+ Start Collection**.
4. Set the Collection ID to `player_profiles`.
5. Set the Document ID to `LeeroyJenkins` *(Note: This must exactly match the SQL username we inserted earlier!)*.
6. Add the first flat field: 
   - Name: `equipped_weapon` | Type: `string` | Value: `Epic Staff`
7. Click **+ Add field** to add our nested object: 
   - Name: `stats` | Type: `map`
8. Inside the new `stats` map, add a nested field: 
   - Name: `str` | Type: `number` | Value: `42`
9. Click **Save**. 

You've just built a nested NoSQL document! When we query this in Python later, we can dynamically pull out their weapon and strength variables.
...
## Part 6: Wiring It All Together on App Engine
...
- This is a handy safety feature to prevent App Engine from spinning up dozens of servers and eating your student credits.
