...
> **Note:** OpenStack does not have all of the features of AWS or GCP, and OpenStack is not provide infrastructure like servers that you can use. OpenStack is like a cloud OS running on Jetstream2's data centers.
...
-   These include more technical, enterprise grade features
...
-   **Custom Private Subnets:** You can create dedicated networks for different parts of your application. For example, you can build a `10.0.1.0/24` subnet strictly for your web servers, and a separate `10.0.2.0/24` subnet strictly for your databases.
...
-   **Inter-VM Communication:** Instead of routing traffic out to the internet and back, your web server VM can talk directly to your database VM over the internal, lightning-fast private OpenStack network.
...
-   **The Paradigm Shift:** If a command can be typed, it can be scripted. Imagine you realize your default firewall rules are insecure and you need to apply a new Security Group to 30 different virtual machines. In a web UI, that takes an hour of tedious clicking. With the CLI, you can write a simple Bash loop:
...
We won't be installing the CLI today, but understanding this paradigm—that the cloud is just an API waiting to receive commands—is the crucial stepping stone toward full DevOps automation.
:::
