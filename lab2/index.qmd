...
/**
 * Use one shared Google provider instance.
 * You can add scopes later if needed (not required for basic sign-in).
 */
const provider = new GoogleAuthProvider();

/**
 * Preferred flow for most desktop browsers.
...
/**
 * Alternative flow for popup-blocked environments.
 * Call signInWithGoogleRedirect() on click, then on page load call
 * handleGoogleRedirectResult() once to finalize.
 */
...
/**
 * Minimal Note shape returned to the UI
 */
export type Note = {
  id: string;
  text: string;
...
/**
 * Subscribe to the current user's notes in realtime.
 * Returns an unsubscribe function.
 *
 * The error callback is optional and will be called if the listener errors
 * (for example, permission-denied or missing-index).
 */
...
/**
 * Demo helper: Subscribe to ALL notes in the collection (no where clause).
 * This intentionally requests every document so you can demonstrate why rules are necessary.
 * Returns an unsubscribe function.
 *
 * The error callback is optional and will be called on permission/index errors.
 */
export function subscribeToAllNotes(
  cb: (notes: Note[]) => void,
  onError?: (err: Error) => void
): () => void {
  // No where() clause — requests entire collection
...
import { addNote, subscribeToNotes, subscribeToAllNotes, deleteNote } from "./notes";
...
export default function App(): React.JSX.Element {
  ...
  // Demo toggle: show only my notes vs show all notes
  // You can hide this behind a debug flag if desired.
  ...
  // Subscribe to notes (either per-user or all)
  useEffect(() => {
    setErrorMsg(null);
    if (!user && !showAll) {
      // nothing to subscribe to when no user and not showing all
      setNotes([]);
      return;
    }
    ...
    const handleError = (err: Error) => {
      ...
      // Show a friendly message (detect common Firestore codes if desired)
      setErrorMsg(err?.message ?? "Unknown error from Firestore listener.");
      setStatus(null);
    };

    if (showAll) {
      unsub = subscribeToAllNotes(
        (fetched) => {
          setNotes(fetched);
          setStatus(null);
        },
  ...
  async function handleAddNote() {
    if (!user) {
      setStatus("You must be signed in to add a note.");
      return;
    }
    ...
    return (
      <div style={{ padding: 20, maxWidth: 640 }}>
        ...
        <p style={{ fontSize: 13, color: "#555" }}>
          <b>Note:</b> If popups are blocked, allow popups or use the redirect flow.
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: 20, maxWidth: 800 }}>
      ...
      <section style={{ marginTop: 18 }}>
        ...
        {/* Demo toggle: show all vs only my notes */}
        <div style={{ marginBottom: 12 }}>
          <label style={{ fontSize: 13, marginRight: 8 }}>
            ...
            />{" "}
            Demo: subscribe to <strong>all</strong> notes (insecure)
          </label>
          <div style={{ fontSize: 12, color: "#666" }}>
            Toggle this to demonstrate why Firestore rules are required.
          </div>
        </div>
        ...
        {status && <div style={{ marginBottom: 12 }}><em>{status}</em></div>}
            ...
            <ul style={{ paddingLeft: 18 }}>
              {notes.map((n) => (
                ...
                >
                  ...
                  <div>
                    <button onClick={() => handleDeleteNote(n.id)} title="Delete note">Delete</button>
                  </div>
                </li>
              ))}
            </ul>
      ...
      <footer style={{ marginTop: 28, fontSize: 13, color: "#555" }}>
        <div><strong>Debug:</strong> UID: {user.uid}</div>
        <div style={{ marginTop: 6 }}>
          Next step: secure your data with Firestore Security Rules so only you can read and write your notes.
        </div>
      </footer>
    </div>
  );
