...
3.  **System Dependencies:** Install the OS-level compiler tools required by Python data science libraries. You must clean the apt cache in the same step to keep the image small.
...
5.  **Security:** Create a non-root user named `developer`, give them ownership of the `/workspace` folder, and switch to that user.
...
## HINTS.TXT — Group A Syntax Guide

* **Installing System Packages & Clearing Cache:**
```dockerfile
RUN apt-get update && apt-get install -y --no-install-recommends build-essential && rm -rf /var/lib/apt/lists/*
```
* **Creating a Non-Root User:**
```dockerfile
RUN useradd --create-home --shell /bin/bash developer && chown -R developer:developer /workspace
USER developer
```
* **Jupyter Startup Command:**
```dockerfile
CMD ["jupyter", "lab", "--ip=0.0.0.0", "--port=8888", "--no-browser", "--NotebookApp.token='lab123'"]
```
:::
...
1.  **Base Image:** Use `python:3.12-slim`.
2.  **Working Directory:** Set it to `/app`.
3.  **Dependencies:** You need `flask` and `pillow` in your `requirements.txt`. Copy your requirements and `server.py` into the container and install the dependencies.
4.  **Security:** Create a non-root user named `appuser`, give them ownership of the `/app` folder, and switch to them before running the app. 
5.  **Configuration:** Set default environment variables: `MODE="noise"` and `SIZE="256"`.
6.  **Network:** The app runs on port `5000`. Expose it.
7.  **Startup:** Run `python server.py`.
...
::: {.callout-tip icon="false"}
## HINTS.TXT — Group B Syntax Guide
