...
-   Instead of giving sequential commands, you write a blueprint (a "state") of exactly what you want the world to look like. - Terraform calculates the difference between reality and your blueprint, and makes the necessary API calls to make reality match your code.
-   **The Syntax:** Terraform uses HashiCorp Configuration Language (HCL). It relies on distinct `blocks`.
-   **Example:**

```         
hcl resource "openstack_compute_instance_v2" "my_server" {    
...
> *Pro-Tip: I highly recommend installing the **Windows Subsystem for Linux (WSL)**. It is the industry standard for cloud engineers using Windows. If you are using WSL, you are running genuine Ubuntu and can just use the Mac/Linux command above*
...
**The Test:**
Now, ask the Jetstream2 API for a list of all servers currently running in our project:
```bash
openstack server list
```
You should see a massive ASCII table displaying your midterm server, my servers, and your classmates' servers! Your laptop has successfully breached the data center perimeter.
:::
...
2.  Paste the following code. **You MUST change the `name` field to include your initials or come up with a unique name!**
...
- Terraform will output an internal IP address (e.g., `10.0.0.X`). 
- Because we didn't attach a public IP, you cannot view this in your laptop's browser. Instead, open the Exosphere web shell for your existing instance in Exosphere
- In web shell run: `curl <THAT_INTERNAL_IP>` 
- You should see the raw HTML output of your custom Nginx page!
...
Congratulations. You have successfully merged Infrastructure with Operations!
:::
